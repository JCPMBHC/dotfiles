  ...
  config = function()
    ...
    ls.add_snippets("react", {
      ...
      const styles = StyleSheet.create({{
        ...
        f(function(_, snip)
          local filename = vim.fn.expand("%:t") -- Obtiene el nombre del archivo actual
          local name = filename:match("^(.-)%.") -- Elimina la extensión

          if name == "index" then
            return "Page"
          else
            -- Elimina guiones y convierte a camelCase
            local camelCaseName = name:gsub("%-+(%w)", function(c)
              return c:upper()
            end)
            -- Capitaliza la primera letra
            return camelCaseName:sub(1, 1):upper() .. camelCaseName:sub(2)
          end
        end),
      })),
